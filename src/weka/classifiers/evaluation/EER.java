...
/**
 * @author Marcelo Class to calculate the EER using points in the Threshold
 *         Curve (ROC)
 * 
 *         Example of usage: IBk ibk=new IBk(5); eval.crossValidateModel(ibk,
 *         data, 10, new Random(1)); ThresholdCurve curve=new ThresholdCurve();
 *         Instances rocPoints= curve.getCurve(eval.predictions()); EER eer=new
 *         EER(rocPoints);
 *         System.out.println("EER= "+eer.calculateEER()*100+"%");
 */

public class EER extends weka.classifiers.evaluation.AbstractEvaluationMetric
    ...
    /**
     * Leaves only the True and False Positives Rates from the Thereshold Curve
     * Instances
     * 
     * @param rocPoints
     *            Instances generated from the getCurve method from the
     *            TheresholdCurve Class
	...
	if (points.size() == 2) {
	    if(points.get(0)==0 && points.get(1)==1){
		return 0.5;
	    }
	    return points.get(0);
	 ...
	 * vector are equal. Specifically, the line through x and y is: # x +
	 * a*(y-x) for all a, and we want a such that # x[1] + a*(y[1]-x[1]) =
	 * x[2] + a*(y[2]-x[2]) so # a = (x[1] - x[2]) / (y[2]-x[2]-y[1]+x[1])
	 */
    ...
    /**
     * Method to get the 2 points in the ROC Curve where X is aproximately
     * equals Y, i.e., FPR=(1-TPR).
     * 
     * @return 4 points: x1,y1,x2,y2. Where x1<y1 and x2>y2 and the difference
     *         between xi,yi is the minimum
     */
	...
	if (FPR.size() > index + 1) {
	    ...
	    if (diff1 < diff2 && !(FPR.get(index - 1) == 1.0)
		    && !(TPR.get(index - 1) == 1.0)) {
    ...
    /**
     * Method to get the 1 points in the ROC Curve where X is aproximately
     * equals Y, i.e., FPR=(1-TPR).
     * 
     * @return 2 points: x,y. Where x and y are aproximately equals, i.e, the
     *         difference between x,y is the minimum
     */
	...
	return "EER                                "
		+ weka.core.Utils.doubleToString(getStatistic(""), 12, 4)
		+ "\n";
    }
